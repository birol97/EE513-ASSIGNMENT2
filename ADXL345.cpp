 ...
 */
std::vector<float>  ADXL345::displayPitchAndRoll(int iterations){
	int count = 0;
        std::vector<float> pitchvalues;
        std::vector<float> rollvalues;
       
while(count < 100){	
                 //storing roll values in vector  
    if(getRoll() != 0){        
               
   rollvalues.push_back(this->getRoll());   //storing roll values in vector
