...
private:
	...
	float pitch, roll;
        float threshold_activity;                                 // in degrees
...
public:
	...
	virtual ADXL345::RESOLUTION getResolution() { return this->resolution; }
        virtual std::vector<float>   getvector(std::vector<float> x,std::vector<float> y);
	...
	virtual std::vector<float>  displayPitchAndRoll(int iterations = 600);
	virtual ~ADXL345();
};
